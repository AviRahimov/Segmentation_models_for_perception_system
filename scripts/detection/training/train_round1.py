"""Round 1 detection training — YOLO11/YOLO26.

Trains a single model variant on the detection dataset using the standard
Ultralytics trainer with freeze=8 (first 8 backbone layers frozen), AdamW,
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="Round 1 detection training (YOLO11 / YOLO26)")
    ...
    p.add_argument("--freeze",  type=int,   default=cfg.get("freeze") or None,
                   help="Layers to freeze. Default: s=8, m=6, l=4 (auto by scale)")
...
def main() -> None:
    ...
    _train_yolo(base_weights, train_kwargs, out_dir, args.model)


def _train_yolo(base_weights: str, train_kwargs: dict[str, Any], out_dir: Path, model_name: str) -> None:
    """Train a standard YOLO11/YOLO26 closed-vocabulary detection model."""
    from ultralytics import YOLO

    logger.info("Loading base weights: %s", base_weights)
    model = YOLO(base_weights)

    logger.info("Starting training (freeze=%d, epochs=%d, batch=%d)...",
